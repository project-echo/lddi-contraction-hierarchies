...
func (h *vertexDistHeap) Push(x vertexDist) {
	*h = append(*h, x)
	h.up(h.Len() - 1)
}
...
func (h *vertexDistHeap) Pop() vertexDist {
	n := h.Len() - 1
	h.Swap(0, n)
	h.down(0, n)
	heapSize := len(*h)
...
// The complexity is O(log n) where n = h.Len().
func (h vertexDistHeap) Remove(i int) vertexDist {
	n := h.Len() - 1
	if n != i {
		h.Swap(i, n)
		if !h.down(i, n) {
...
func (h vertexDistHeap) up(j int) {
	for {
		i := (j - 1) / 2 // parent
		if i == j || !h.Less(j, i) {
			break
...
func (h vertexDistHeap) down(i0, n int) bool {
	i := i0
	for {
		j1 := 2*i + 1
