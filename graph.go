...
type Graph struct {
	...
	restrictions map[int64]map[int64]bool
...
func (graph *Graph) AddTurnRestriction(from, to int64) error {
	if graph.frozen {
		return ErrGraphIsFrozen
	}
	...
	if graph.restrictions == nil {
		graph.restrictions = make(map[int64]map[int64]bool)
	}

	if _, ok := graph.restrictions[from]; !ok {
		graph.restrictions[from] = make(map[int64]bool)
	}

	graph.restrictions[from][to] = true
